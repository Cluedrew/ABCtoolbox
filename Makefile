...
$(LIBDIR)/%.hpp : $(SORDIR)/%/$$*.hpp | $(LIBDIR)
	cp $< $@
# Check for a matching tpp: remove old and create new.
#	[ -e $(patsubsut %.hpp,%.tpp,$@) ] && rm $(..., $@) ; ...
#	... ; [ -e $(patsubsut %.hpp,%.tpp,$<) ] && cp $(..., $<) $(..., $@)
# This is crude but I don't know else to treat the as a 'unit' right now.
