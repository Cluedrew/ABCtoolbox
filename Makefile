...
$(OBJDIR)/%.o : \
$(patsubst $(CODEDIR)/\%/%.c,$(OBJDIR)/\%/%.o,$(wildcard,$(CODEDIR)/%/*.c)
	$(LINK) $(LINKFLAGS) $^ $(object_LIB) -o $@
# Still some problems getting the valibles correct. And the libraries linked.
# Also how to differentate it from the intermediate objects.

# Rule for intermediate objects:
# (all objects generated directly from code files)
$(OBJDIR)/%.o : $(CODEDIR)/%.cpp | $(OBJDIR)/$(dir %)
	$(CXX) $(CXXFLAGS) -c $< -o $@
